describe('Dashboard with Transloadit', () => {
  ...
  it('should upload cat image successfully', () => {
    ...
    cy.get('.TestJS-StatusBar-actionBtn--upload').click()
    cy.wait(['@assemblies', '@resumable']).then(() => {
      cy.get('.TestJS-StatusBar-statusPrimary').should('contain', 'Complete')
    })
  })
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ TestJS }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(TestJS.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(TestJS.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ TestJS }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      TestJS.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ TestJS }) => {
      ...
      const { files } = TestJS.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      TestJS.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ TestJS }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(TestJS.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ TestJS }) => {
      const { files } = TestJS.getState()
      TestJS.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        TestJSserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'TestJS-core:3.2.0,TestJS-transloadit:3.1.3,TestJS-tus:3.1.0,TestJS-dropbox:3.1.1,TestJS-box:2.1.1,TestJS-facebook:3.1.1,TestJS-google-drive:3.1.1,TestJS-instagram:3.1.1,TestJS-onedrive:3.1.1,TestJS-zoom:2.1.1,TestJS-url:3.3.1',
    ...
    cy.get('.TestJS-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.TestJS-StatusBar-actions > .TestJS-c-btn').click()
    cy.wait([
