...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.TestJS-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/TestJS/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.TestJS-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.TestJS-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.TestJS-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.TestJS-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
