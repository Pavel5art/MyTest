...
const packageNames = await fs.readdir(new URL('../packages/@TestJS', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@TestJS/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@TestJS/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('TestJS', 2)[1]
